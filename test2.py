...
def test_claims():
    ...
    with open("cert2.pem", "r") as f:
        public_key = f.read()

    # 2. Load the Raw JWT string
    with open("sample-jwt2.txt", "r") as f:
    ...
    assert (
        has_perm_shared_2
    ), "Expected permission_dhilipsiva_2 in group_test_dhilipsiva2"
    ...
    assert (
        has_perm_3_in_g2
    ), "Expected permission_dhilipsiva_3 in group_test_dhilipsiva2"
    ...
    assert (
        not has_perm_3_in_g1
    ), "Expected permission_dhilipsiva_3 NOT in group_test_dhilipsiva1"
    ...
    print(f"   get_default_org_short_code: {user.get_default_org_short_code()}")

    assert user.get_default_org_short_code() == "T-2345"
    ...
    assert set(user.org_short_codes) == {
        "group_test_dhilipsiva1",
        "group_test_dhilipsiva2",
    }
